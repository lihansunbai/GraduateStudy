...
class point extends server {
    ...
    protected function todatabase() {
        ...
        $sql = 'INSERT INTO point(userid,pointid,x,y)
                VALUES($1,$2,$3,$4);';

        $result = pg_query_params($this->_connection, $sql, array(
            $this->_request->username,
            $this->pointid,
            $this->_request->points[0][1][0],
            $this->_request->points[0][1][1]
        ));
        ...
        if (!$result) {
            ...
            $this->makeResponse(true, "save poits:" . $this->_request->points[0][0] . " ");
        }
    ...
    protected function getpointinfo() {
        ...
        $sql = "SELECT MAX(pointid) FROM point"
